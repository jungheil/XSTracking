...
UVC::UVC(int cam) {
    ...
    if (fb_.isOpened()) { cam_id_ = cam_count_++; }
}
...
UVC::UVC(std::string path, bool time_stamp) : time_stamp_(time_stamp) {
    ...
    ret       = fb_.isOpened();
    if (time_stamp_) { ret &= TSInit(path); }
    if (ret) { cam_id_ = cam_count_++; }
}
...
bool UVC::TSInit(std::string path) {
    ...
    // avformat_open_input(&ifmt_ctx, in_filename,
    // 0,0)；用来打开文件，解封文件头，若成功，返回0。
    ...
    for (i = 0; i < ifmt_ctx_->nb_streams; i++) {
        if (ifmt_ctx_->streams[i]->codecpar->codec_type ==
            AVMEDIA_TYPE_VIDEO)  //如果是视频类型
        {
            videoindex_ = i;
    ...
    while (retry++ < 30) {
        ...
        if (pkt.stream_index == videoindex_) {
            ...
            if ((pkt.data[4] & 0x1F) == 6) {
                int k = 6;
                while (pkt.data[k - 1] != 0x54 || pkt.data[k] != 0x4D) { k++; }
                k++;
                XSTime  xs_sei_time;
...
std::shared_ptr<Camera> CameraFactory::CreateCamera(CAMERA_TYPE cam_type,
                                                    std::string path) {
    switch (cam_type) {
