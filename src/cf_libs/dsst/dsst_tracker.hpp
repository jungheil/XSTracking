...
class DsstTracker : public CfTracker {
  public:
    ...
    DsstTracker(DsstParameters paras, DsstDebug<T>* debug = 0)
        ...
        if (CV_MAJOR_VERSION < 3) {
            std::cout << "DsstTracker: Using OpenCV Version: "
                      << CV_MAJOR_VERSION << std::endl;
            std::cout << "For more speed use 3.0 or higher!" << std::endl;
        }
    }

    virtual ~DsstTracker() { delete _scaleEstimator; }

    virtual bool reinit(const cv::Mat& image, cv::Rect_<int>& boundingBox) {
        Rect bb =
            Rect(static_cast<T>(boundingBox.x), static_cast<T>(boundingBox.y),
    ...
    virtual bool update(const cv::Mat& image, cv::Rect_<int>& boundingBox) {
        ...
        if (update_(image, bb) == false) return false;
    ...
    virtual bool updateAt(const cv::Mat&     image,
    ...
    virtual TrackerDebug* getTrackerDebug() { return _debug; }
  ...
  private:
    ...
    bool reinit_(const cv::Mat& image, Rect& boundingBox) {
        ...
        _pos.y =
            floor(boundingBox.y) + floor(boundingBox.height * consts::c0_5);
        ...
        if (!_ORIGINAL_VERSION) {
            ...
            _templateSz = Size(floor(_templateSz.width / _scale),
                               floor(_templateSz.height / _scale));
        }

        _baseTargetSz =
            Size(targetSize.width / _scale, targetSize.height / _scale);
        ...
        T outputSigma =
            sqrt(_templateSz.area() / ((1 + _PADDING) * (1 + _PADDING))) *
    ...
    bool getTranslationTrainingData(const cv::Mat&        image,
                                    std::shared_ptr<DFC>& hfNum, cv::Mat& hfDen,
    ...
    bool getTranslationFeatures(const cv::Mat&        image,
                                std::shared_ptr<DFC>& features,
                                const Point& pos, T scale) const {
        cv::Mat patch;
        Size    patchSize = _templateSz * scale;

        if (getSubWindow(image, patch, patchSize, pos) == false) return false;
        ...
        if (_debug != 0) _debug->showPatch(patch);

        cv::Mat floatPatch;
        patch.convertTo(floatPatch, CV_32FC(3));
        normalize(patch, patch, 0, 255, cv::NORM_MINMAX);
        ...
        if (patch.channels() == 1) {
            if (_CELL_SIZE != 1)
                resize(patch, patch, features->channels[0].size(), 0, 0,
                       _RESIZE_TYPE);

            features->channels[DFC::numberOfChannels() - 1] =
                patch / 255.0 - 0.5;
        } else {
            if (_CELL_SIZE != 1)
                resize(patch, patch, features->channels[0].size(), 0, 0,
    ...
    // Add update_at, when we want to update the model with new boudingbox, we
    // set it to true.
    bool updateAtScalePos(const cv::Mat& image, const Point& oldPos,
                          const T oldScale, Rect& boundingBox,
                          const bool update_at = false) {
        ++_frameIdx;
        ...
        if (!update_at) {
            if (detectModel(image, response, maxResponseIdx, newPos,
                            newScale) == false)
                return false;
        }
        // return box
        ...
        if (_ENABLE_TRACKING_LOSS_DETECTION) {
            if (evalReponse(image, response, maxResponseIdx, tempBoundingBox) ==
                false)
                return false;
        }

        if (updateModel(image, newPos, newScale) == false) return false;

        boundingBox &=
            Rect(0, 0, static_cast<T>(image.cols), static_cast<T>(image.rows));
    ...
    bool evalReponse(const cv::Mat& image, const cv::Mat& response,
        ...
        T peakValue = 0;
        T psrClamped =
            calcPsr(response, maxResponseIdx, _PSR_PEAK_DEL, peakValue);
        ...
        if (psrClamped < _PSR_THRESHOLD) return false;
    ...
    bool detectModel(const cv::Mat& image, cv::Mat& response,
        ...
        idft(responseTf, translationResponse,
        ...
        T posDeltaX =
            (subDelta.x + 1 - floor(translationResponse.cols / consts::c2_0)) *
            newScale;
        T posDeltaY =
            (subDelta.y + 1 - floor(translationResponse.rows / consts::c2_0)) *
            newScale;
        newPos.x += round(posDeltaX * _CELL_SIZE);
        newPos.y += round(posDeltaY * _CELL_SIZE);

        if (_debug != 0) _debug->showResponse(translationResponse, maxResponse);

        if (_scaleEstimator) {
            // find scale
            T tempScale = newScale * _templateScaleFactor;

            if (_scaleEstimator->detectScale(image, newPos, tempScale) == false)
                return false;
    ...
    bool updateModel(const cv::Mat& image, const Point& newPos, T newScale) {
        ...
        _hfDenominator =
            (1 - _LEARNING_RATE) * _hfDenominator + _LEARNING_RATE * hfDen;
        ...
        if (_scaleEstimator) {
            if (_scaleEstimator->updateScale(
                    image, newPos, newScale * _templateScaleFactor) == false)
                return false;
        }
  ...
  private:
    typedef void (*cvFhogPtr)(const cv::Mat&        img,
                              std::shared_ptr<DFC>& cvFeatures, int binSize,
                              int fhogChannelsToCopy);
    cvFhogPtr cvFhog = 0;

    typedef void (*dftPtr)(const cv::Mat& input, cv::Mat& output, int flags);
    dftPtr calcDft = 0;
    ...
    T _templateScaleFactor;  // _templateScaleFactor is used to calc the target
                             // scale
