...
template <typename T>
T subPixelPeak(T* p) {
    T delta = mat_consts::constants<T>::c0_5 * (p[2] - p[0]) /
              (2 * p[1] - p[2] - p[0]);
...
cv::Point_<T> subPixelDelta(const cv::Mat& response, const cv::Point2i& delta) {
    cv::Point_<T> subDelta(static_cast<float>(delta.x),
    ...
    for (int i = -1; i < 2; ++i) {
        vNeighbors[i + 1] =
            response.template at<T>(mod(delta.y + i, response.rows), delta.x);
        hNeighbors[i + 1] =
            response.template at<T>(delta.y, mod(delta.x + i, response.cols));
    }
...
cv::Mat gaussianShapedLabels2D(T sigma, const cv::Size_<T>& size) {
    ...
    for (int row = 0; row < height; ++row) {
        ...
        for (int col = 0; col < width; ++col) {
            rsd[row * width + col] =
                exp((colValues[col] + rowValue) * sigmaMult);
        }
    }
...
template <typename T>
void divideSpectrumsNoCcs(const cv::Mat& numerator, const cv::Mat& denominator,
                          cv::Mat& dst) {
    ...
    // TODO: this is still very slow => vectorize (note that mulSpectrums is not
    // vectorized either...)
...
template <typename T>
bool getSubWindow(const cv::Mat& image, cv::Mat& patch,
                  const cv::Size_<T>& size, const cv::Point_<T>& pos,
                  cv::Point_<T>* posInSubWindow = 0) {
    int width  = static_cast<int>(size.width);
    int height = static_cast<int>(size.height);
    ...
    if (subWindow.cols == 0 || subWindow.rows == 0) return false;
    ...
    if (diffTopX > 0 || diffTopY > 0 || diffBottomX < 0 || diffBottomY < 0) {
        ...
        copyMakeBorder(subWindow, subWindow, diffTopY, -diffBottomY, diffTopX,
                       -diffBottomX, cv::BORDER_CONSTANT);
    }

    // this if can be true if the sub window
    // is completely outside the image
    if (width != subWindow.cols || height != subWindow.rows) return false;
