...
class FeatureChannels_ {
  public:
    static void mulValueFeatures(std::shared_ptr<FeatureChannels_>& m,
                                 const T                            value) {
        for (int i = 0; i < NUMBER_OF_CHANNELS; ++i) m->channels[i] *= value;
    }
    ...
    static cv::Mat sumFeatures(const std::shared_ptr<FeatureChannels_>& x) {
        ...
        for (int i = 1; i < NUMBER_OF_CHANNELS; ++i) res += x->channels[i];

        return res;
    }

    static cv::Mat sumFeaturesInPlace(
        const std::shared_ptr<FeatureChannels_>& x) {
    ...
    static std::shared_ptr<FeatureChannels_> idftFeatures(
        ...
        for (int i = 0; i < NUMBER_OF_CHANNELS; ++i)
            idft(features->channels[i], res->channels[i],
                 cv::DFT_REAL_OUTPUT | cv::DFT_SCALE, 0);

        return res;
    }

    static T squaredNormFeaturesCcs(
        const std::shared_ptr<FeatureChannels_>& Af) {
    ...
    static T squaredNormFeaturesNoCcs(
        const std::shared_ptr<FeatureChannels_>& Af) {
        int     n    = Af->channels[0].rows * Af->channels[0].cols;
        T       sum_ = 0;
        cv::Mat elemMul;
    ...
    static std::shared_ptr<FeatureChannels_> mulSpectrumsFeatures(
        const std::shared_ptr<FeatureChannels_>& Af,
        const std::shared_ptr<FeatureChannels_>& Bf, bool conjBf) {
        std::shared_ptr<FeatureChannels_> resf(new FeatureChannels_());

        for (int i = 0; i < NUMBER_OF_CHANNELS; ++i)
            mulSpectrums(Af->channels[i], Bf->channels[i], resf->channels[i], 0,
                         conjBf);
    ...
    static std::shared_ptr<FeatureChannels_> mulSpectrumsFeatures(
        const cv::Mat& Af, const std::shared_ptr<FeatureChannels_>& Bf,
        bool conjBf = false) {
        std::shared_ptr<FeatureChannels_> resf(new FeatureChannels_());
    ...
    static const int numberOfChannels() { return NUMBER_OF_CHANNELS; }
