...
/* This class represents the 1D correlation filter proposed in  [1]. It is used
to estimate the scale of a target.
...
class ScaleEstimator {
  public:
    ...
    ScaleEstimator(ScaleEstimatorParas<T> paras)
        ...
        if (_DEBUG_OUTPUT) {
            if (CV_MAJOR_VERSION < 3) {
                std::cout << "ScaleEstimator: Using OpenCV Version: "
    ...
    bool reinit(const cv::Mat& image, const Point& pos, const Size& targetSize,
        ...
        _scaleModelSz.height = static_cast<T>(
            std::max(_SCALE_CELL_SIZE + 1, int(_scaleModelSz.height)));
        _scaleModelSz.width = static_cast<T>(
            std::max(_SCALE_CELL_SIZE + 1, int(_scaleModelSz.width)));

        // expand ysf to have the number of rows of scale samples
        //            Size tmp_feature_size;
        //            tmp_feature_size.width = std::max(1,
        //            int(floor(_scaleModelSz.width / _SCALE_CELL_SIZE)));
        //            tmp_feature_size.height = std::max(1,
        //            int(floor(_scaleModelSz.height / _SCALE_CELL_SIZE))); int
        //            ysfRow = static_cast<int>(tmp_feature_size.width *
        //            tmp_feature_size.height * FFC::numberOfChannels());
        int ysfRow =
            static_cast<int>(floor(_scaleModelSz.width / _SCALE_CELL_SIZE) *
        ...
        if (getScaleTrainingData(image, pos, currentScaleFactor, sfNum,
                                 sfDen) == false)
            return false;
        // 初始化最大最小的scale
        cv::Size imszie = image.size();
        _MIN_SCALE_FACTOR =
            std::pow(_SCALE_STEP, ceil(log(std::max(5.0 / _targetSize.width,
                                                    5.0 / _targetSize.height)) /
                                       log(_SCALE_STEP)));
        _MAX_SCALE_FACTOR = std::pow(
            _SCALE_STEP,
            floor(log(std::min(imszie.width / (float)_targetSize.width,
                               imszie.height / (float)_targetSize.height)) /
                  log(_SCALE_STEP)));
    ...
    bool detectScale(const cv::Mat& image, const Point& pos,
        ...
        idft(responseSf, scaleResponse,
             cv::DFT_REAL_OUTPUT | cv::DFT_SCALE | cv::DFT_ROWS);
    ...
    bool updateScale(const cv::Mat& image, const Point& pos,
        ...
        _sfDenominator =
            (1 - _LEARNING_RATE) * _sfDenominator + _LEARNING_RATE * sfDen;
        _sfNumerator =
            (1 - _LEARNING_RATE) * _sfNumerator + _LEARNING_RATE * sfNum;
        return true;
    }

  private:
    bool getScaleTrainingData(const cv::Mat& image, const Point& pos,
                              const T& currentScaleFactor, cv::Mat& sfNum,
                              cv::Mat& sfDen) const {
        cv::Mat xs;
        if (getScaleFeatures(image, pos, xs, currentScaleFactor) == false)
            return false;
    ...
    bool getScaleFeatures(const cv::Mat& image, const Point& pos,
        ...
        // extract first required sub window separately; smaller scales are
        // extracted from this patch to avoid multiple border replicates on out
        // of image patches
        int idxScale = 1;
        ...
        if (getSubWindow(image, firstPatch, firstPatchSize, pos,
                         &posInFirstPatch) == false)
            return false;

        if (_ORIGINAL_VERSION)
            depResize(firstPatch, patchResized, _scaleModelSz);
        else
            cv::resize(firstPatch, patchResized, _scaleModelSz, 0, 0,
                       _RESIZE_TYPE);

        patchResized.convertTo(patchResizedFloat, CV_32FC(3));
        fhogToCvCol(patchResizedFloat, features, _SCALE_CELL_SIZE, idxScale,
                    cosFactor);
        ...
        for (idxScale = 2; idxScale < _N_SCALES - 1; ++idxScale) {
            ...
            if (getSubWindow(firstPatch, patch, patchSize, posInFirstPatch) ==
                false)
                return false;
            ...
            if (_ORIGINAL_VERSION)
                ...
                cv::resize(patch, patchResized, _scaleModelSz, 0, 0,
                           _RESIZE_TYPE);
  ...
  private:
    typedef void (*fhogToCvRowPtr)(const cv::Mat& img, cv::Mat& cvFeatures,
                                   int binSize, int rowIdx, T cosFactor);
    ...
    // it should be possible to find more reasonable values for min/max scale;
