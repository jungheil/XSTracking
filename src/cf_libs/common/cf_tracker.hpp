...
class CfTracker {
  public:
    ...
    virtual bool updateAt(const cv::Mat&  image,
                          cv::Rect_<int>& boundingBox)                     = 0;

    virtual bool update(const cv::Mat&    image,
                        cv::Rect_<float>& boundingBox)   = 0;
    virtual bool reinit(const cv::Mat&    image,
                        cv::Rect_<float>& boundingBox)   = 0;
    virtual bool updateAt(const cv::Mat&    image,
                          cv::Rect_<float>& boundingBox) = 0;

    virtual bool update(const cv::Mat&     image,
                        cv::Rect_<double>& boundingBox)   = 0;
    virtual bool reinit(const cv::Mat&     image,
                        cv::Rect_<double>& boundingBox)   = 0;
    virtual bool updateAt(const cv::Mat&     image,
                          cv::Rect_<double>& boundingBox) = 0;
